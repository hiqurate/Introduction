...
const CoverSlide = () => {
  return (
    <div className="min-h-screen w-full bg-slate-900 font-sans text-white relative flex flex-col lg:flex-row overflow-visible items-stretch">
      ...
      {/* Top Bar */}
      <div className="absolute top-0 left-0 w-full h-1 bg-cyan-500 z-50" />

      {/* Centered inner wrapper to control max width and responsiveness; stack text above image on small screens to avoid cropping */}
      <div className="relative z-10 w-full max-w-[1000px] mx-auto flex flex-col-reverse lg:flex-row items-stretch">
        {/* Left content section (allow shrinking with min-w-0) */}
        <section className="w-full lg:w-7/12 min-w-0 h-auto flex flex-col justify-center px-6 py-8 md:py-12 lg:pl-20 lg:pr-12 z-10 relative bg-slate-900/95 lg:bg-slate-900/95">
          {/* Logo Area */}
          ...
          <div className="mt-8">
            ...
            <div className="inline-flex items-center gap-2 px-3 py-1 border border-cyan-500/30 rounded-full bg-slate-800/50 mb-6">
              ...
              <p className="text-xs font-mono text-cyan-400 tracking-widest uppercase">{SITE_DATA.hero.badge}</p>
            </div>

            {/* H1 Title (fluid/clamped to avoid overflowing) */}
            <h1 className="text-[clamp(1.6rem,6vw,3.75rem)] md:text-[clamp(2rem,4.5vw,4.5rem)] font-extrabold leading-tight mb-4 tracking-tight">
              {SITE_DATA.hero.titleLine1} <br />
              <span
                className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-cyan-200"
          ...
          {/* Divider */}
          <div className="w-24 h-1.5 bg-cyan-500 my-8 rounded-full" />
          ...
          <div className="max-w-2xl">
            ...
            <div className="flex items-start gap-4 p-4 bg-slate-800/40 border-l-2 border-cyan-500 rounded-r-lg">
              ...
              <p className="text-sm text-slate-400">
                Founded and led by industry practitioners, bridging the gap between{' '}
                <strong className="text-white">enterprise-grade security</strong> and{' '}
                <strong className="text-white">real-world business needs</strong>.
              </p>
            </div>
          </div>
        ...
        {/* Right visual section (allow shrinking with min-w-0; image won't crop on small screens) */}
        <aside className="w-full lg:w-5/12 min-w-0 h-auto md:h-[360px] relative bg-slate-800 flex flex-col">
          <div className="absolute inset-0 z-0">
            <img
              alt="Cybersecurity Abstract"
              className="w-full h-full min-h-[220px] md:min-h-[320px] object-contain md:object-cover object-center opacity-90"
          ...
          {/* Floating UI Card - responsive: full width on small, absolute on md+ */}
          <div className="relative md:absolute bottom-0 md:bottom-16 right-0 md:right-16 w-full md:w-64 p-4 bg-slate-900/90 backdrop-blur-sm border border-cyan-500/30 rounded-lg shadow-2xl z-20 mx-4 md:mx-0 mb-6 md:mb-0 self-center md:self-end">
          ...
          {/* Decorative Elements */}
          <div className="absolute top-10 right-10 w-20 h-20 border-t-2 border-r-2 border-cyan-500 opacity-50 hidden md:block" />
          <div className="absolute bottom-10 right-10 w-8 h-8 border-b-2 border-r-2 border-white opacity-20 hidden md:block" />
        </aside>
      </div>
    </div>
  );
